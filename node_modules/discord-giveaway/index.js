/* To know more information about this npm module, go here : https://npmjs.com/package/discord-giveaway */
...
module.exports = {
    ...
    launch(client, options){
        ...
        if(options.embedColor){
            ...
            if(isNaN(number)){
                throw new TypeError(options.embedColor+" Color should be in hex format.");
            }
        ...
        return new Promise(function(resolve, reject){
            ...
            if(!options.messages){
                options.messages = {
                    ...
                    embedFooter: "T-Shirt",
                    noWinner: "Giveaway cancelled, nobody reacted.",
                    winners: "Winner(s)",
                    endedAt: "Ended at",
            ...
            if(!guildChannel){
                return reject(guildChannel+" is not a valid channel.");
            }
            if(!options.time){
                return reject(options.time+" is not a valid time.");
            }
            if(!options.prize){
                return reject(options.prize+" is not a string.");
            }
            if(!options.winnersCount){
                return reject(options.winnersCount+" is not a valid number.");
            }
    ...
    async reroll(messageID, options){
        return new Promise(async function(resolve, reject){
            ...
            if(!options){
                options = {
                    congrat: "🎉 Congratulations! {winners} are the new winner(s).",
                    error: "Nobody reacted, winners couldn't be defined."
                }
            }
            let giveaways = require(settings.storage);
