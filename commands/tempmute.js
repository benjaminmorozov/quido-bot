...
exports.run = async (client, message, args) => {
    ...
    if (message.member.roles.has(OwnerRole.id) || message.member.roles.has(DiscordManagerRole.id)) {
      ...
      message.channel.send(`<@${member.id.tag}> has been muted by ${message.author.tag} for a duration of ${ms(ms(mutetime))} for: ${reason}`);
      member.send(`<@${member.id.tag}>, you have been muted by ${message.author.tag} for a duration of ${ms(ms(mutetime))} for: ${reason}`)
      ...
      await(member.addRole(muterole.id));
      message.channel.send(`<@${member.id.tag}> has been muted by ${message.author.tag} for a duration of ${ms(ms(mutetime))} for: ${reason}`);
      message.member.send(`<@${member.id.tag}>, you have been muted by ${message.author.tag} for a duration of ${ms(ms(mutetime))} for:${reason}`)
